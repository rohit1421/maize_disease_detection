...
def get_model():
    global model
    model = load_model(r'C:\Users\rahma\OneDrive\Desktop\project\Maize-Diseases-Detection\Traning\model_new.keras')
    print(" * Model loaded!")
...
def check(path):
    ...
    if 0 <= index < len(class_labels):
        ...
        return [-1, 0]  # Return an invalid index to indicate an error
        ...
        if file:
            try:
                if file and allowed_file(file.filename):
                    ...
                    if result[0] == -1:
                        return render_template('predict.html', error="Invalid image. Please upload an image recognized by the model.")
